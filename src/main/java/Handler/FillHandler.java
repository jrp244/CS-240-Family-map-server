    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            if (exchange.getRequestMethod().toLowerCase().equals("post")) {
                ...
                String uri = exchange.getRequestURI().toString(); //Gets the endpoint
                String[] array = uri.split("/");
                userName = array[2];
                generationNum = Integer.parseInt(array[3]);
                Gson gson = new Gson();
                FillService service = new FillService();
                FillResult result = service.fill(userName, generationNum);
